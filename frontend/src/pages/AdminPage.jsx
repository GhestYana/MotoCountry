  ...
  const handleBlock = async (userId) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(data.message || 'Помилка при блокуванні');
      }
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="admin-users">
      ...
      {loading ? (
        ...
        <table className="admin-table">
          ...
          <tbody>
            {filtered.map(u => (
              <tr key={u.id} className={u.is_blocked ? 'user-row-blocked' : ''}>
                ...
                <td>
                  <span className={u.is_email_verified ? 'verified-yes' : 'verified-no'}>
                    {u.is_email_verified
                      ? <><Check size={14} /> Так</>
                      : <><X size={14} /> Ні</>}
                  </span>
                ...
                <td>
                  {u.is_blocked
                    ? <span className="user-status-badge blocked"><Lock size={13} /> Заблоковано</span>
                    : <span className="user-status-badge active"><Check size={13} /> Активний</span>
                  }
                </td>
                <td>
                  <div className="table-actions">
                    {u.is_blocked ? (
                      ...
                      <button
                        className="block-btn"
                        onClick={() => {
                          if (window.confirm(`Заблокувати ${u.first_name} ${u.last_name}?`)) {
                            handleBlock(u.id);
                          }
                        }}
                        disabled={u.role === 'admin'}
                        title={u.role === 'admin' ? 'Не можна заблокувати адміна' : ''}
                      >
                        Заблокувати
                      </button>
                    )}
