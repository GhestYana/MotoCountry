...
const ProductDetailPage = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const res = await fetch('/api/cart-items/add-item', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ userId, prodId: id, category: category, quantity: 1 })
      });
  ...
  const handleFavorite = async () => {
    ...
    if (!normalizedCategory) {
      console.error('Cannot add to favorites: unknown category', category);
      return;
    }
    ...
    if (shouldAdd) {
      favorites = [...favorites.filter((fav) => String(fav.id) !== String(id)), item];
    } else {
    ...
    const token = localStorage.getItem('token');
    try {
      const endpoint = shouldAdd ? '/api/favorites/add' : '/api/favorites/remove';
      await fetch(endpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ prodId: id, category: normalizedCategory }),
      });
    } catch (err) {
      console.error('Error syncing favorite to server:', err);
    }
  };
                        ...
                        <div className="review-rating-edit">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                              ...
                              fill={editRating >= star ? "#ffc107" : "none"}
                              stroke={editRating >= star ? "#ffc107" : "currentColor"}
                            />
                          ))}
                        </div>
                        <textarea
                      ...
                      <>
                        <div className="review-card-header">
                          ...
                          <div className="review-rating">
                            {[...Array(5)].map((_, i) => (
                              <Star key={i} size={16} className={`star-icon ${i < r.rating ? 'filled' : ''}`} fill={i < r.rating ? "#ffc107" : "none"} stroke={i < r.rating ? "#ffc107" : "currentColor"} />
                            ))}
                          </div>
                        </div>
